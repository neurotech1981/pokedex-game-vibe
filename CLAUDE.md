...
- **League Challenge**: `src/data/league.ts` — 8 Kanto gyms → Elite Four → Champion Blue, strict linear unlocks (`isStageUnlocked`), level floors 52→80 scaling with player avg, rosters via cached `fetchLeagueTeam` (PokeAPI), XP ×2, badges persisted in `profile.league.defeated`. Trainer portraits bundled in `public/assets/trainers/` (Showdown). UI: `LeagueCard` in BattleSetup, portrait chip in battle, badge labels in PostBattlePanel.
